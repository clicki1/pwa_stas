...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4 class="m-0">Редактировать данные</h4>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                <form  id="prod_form" action="{{ route('admin.product.update', $product->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <div class="form-group pt-1">
                        <input type="number"  id="briquette"  name="briquette" value="{{  $product->briquette ?? old('briquette') }}"
                               class="form-control"
                               placeholder="Сырого брикета (день), кг">
                    </div>
                    <div class="form-group pt-1">
                        <input type="number"  id="bake"   name="bake" value="{{  $product->bake ?? old('bake') }}"
                               class="form-control"
                               placeholder="Количество заложенного в печь (день), кг">
                    </div>
                    <div class="form-group pt-1">
                        <input type="number"  id="packed_1"   name="packed_1" value="{{  $product->packed_1 ?? old('packed_1') }}"
                               class="form-control"
                               placeholder="Упаковано 1 сорт (день), кг">
                    </div>
                    <div class="form-group pt-1">
                        <input type="number"  id="packed_2"  name="packed_2" value="{{  $product->packed_2 ?? old('packed_2') }}"
                               class="form-control"
                               placeholder="Упаковано 2 сорт (день), кг">
                    </div>
                    <div class="form-group pt-3">
                        <button type="submit" class="btn btn-primary btn-lg" id="prod_btn">Редактировать</button>
                    </div>
                </form>
            </div>
            <!-- /.row -->
